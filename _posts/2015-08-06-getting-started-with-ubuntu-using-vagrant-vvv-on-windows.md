...
3. Follow VVV recommended step-by-step installation 
...
5. If you want to be able to use other devices (e.g. mobile phone) to access the virtual machine, you need to configure Vagrant with `config.vm.network :public_network` as recommended by [webdevstudios](http://webdevstudios.com/2015/08/04/how-to-view-a-locally-developed-website-devices/).
6. You can now use the Vagrant environment with `vagrant up` or stop it with `vagrant halt`
7. Visit [http://vvv.dev/](http://vvv.dev/) for accessing VVV dashboard
